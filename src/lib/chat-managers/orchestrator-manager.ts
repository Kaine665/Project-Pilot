...
2. **文件独占原则（关键）：**子任务之间绝对不能创建或修改同一个文件。每个文件只能归属于一个子任务。
   - 共享配置文件（package.json、tsconfig.json、.gitignore 等）必须由最先执行的基础设施任务统一创建
   - 后续任务如果需要修改这些文件（如添加 bin 字段），必须用 dependsOn 依赖基础任务，在它的分支基础上修改
   - 类型定义文件（如 types/）同理，只由一个任务创建，其他任务通过依赖关系继承
...
8. 在 description 中明确列出该任务负责创建/修改的文件清单，确保文件清单在所有任务间无交集
${teamSection}
**输出格式（必须是有效的 JSON）：**
