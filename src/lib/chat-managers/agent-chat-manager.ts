...
class AgentChatManager {
  ...
  private async finalizeRun(run: AgentChatRun, aborted: boolean): Promise<void> {
    ...
    if (run.assistantText) {
      ...
      const satelliteCtx: SatelliteContext = {
        ...
        assistantTurnCount,
        runStatus: run.status === 'running' ? (aborted ? 'stopped' : 'completed') : run.status,
        ...
        resumeSession: (message: string) => {
          // Defer resume to after finalization + persist completes
          setTimeout(async () => {
            ...
            try {
              ...
              console.log(`${LOG_PREFIX} Satellite resumed session ${run.sessionId}`);
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err);
              if (msg.includes('already running')) {
                console.log(`${LOG_PREFIX} Session ${run.sessionId} already running (race), skip`);
              } else {
      ...
      // Fallback title if satellites didn't set one
      if (!run.sessionTitle) {
  ...
  private async persistAfterClose(run: AgentChatRun, _aborted: boolean): Promise<void> {
    ...
    // Health guard is now handled by satellite tasks in finalizeRun()
  }
