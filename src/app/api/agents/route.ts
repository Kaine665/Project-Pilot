...
export async function POST(request: NextRequest) {
  const { name, description, systemPrompt, icon, capabilities, requiredParams, contextIds, defaultResources, projectKey, triggerHints, defaultProvider, defaultModel } = await request.json();
  if (!name?.trim()) {
...
export async function PATCH(request: NextRequest) {
  ...
  const { id, name, description, systemPrompt, icon, capabilities, requiredParams, contextIds, defaultResources, projectKey, triggerHints, defaultProvider, defaultModel } = body;
  if (!id) {
  ...
  if (defaultProvider !== undefined) agent.defaultProvider = defaultProvider || undefined;
  if (defaultModel !== undefined) agent.defaultModel = defaultModel?.trim() || undefined;
