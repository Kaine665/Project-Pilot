...
import { ListTodo, Plus, Trash2, ChevronDown } from 'lucide-react';
...
export default function TodosPage() {
  ...
  const handleUpdate = async (id: string, updates: Partial<Pick<TodoItem, 'title' | 'description' | 'status' | 'priority'>>) => {
    try {
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-[1000px] px-6 py-10">
        ...
        <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_90px_90px_120px_40px] border-b border-zinc-200 bg-zinc-50 px-4 py-2.5 text-xs font-medium text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-400">
            ...
            <div>
              {visibleTodos.map(todo => (
                <div key={todo.id} className="group">
                  {/* Main row */}
                  <div
                    className={`grid grid-cols-[1fr_90px_90px_120px_40px] items-center px-4 py-2.5 border-b border-zinc-100 transition-colors hover:bg-zinc-50 dark:border-zinc-800/50 dark:hover:bg-zinc-900/40 ${
                  ...
                  {expandedId === todo.id && (
                    <div className="border-b border-zinc-100 bg-zinc-50/50 px-4 py-3 pl-12 dark:border-zinc-800/50 dark:bg-zinc-900/20">
          ...
          {/* Bottom add button — always visible when not creating */}
          {!isCreating && !loading && (
            <button
              onClick={() => setIsCreating(true)}
