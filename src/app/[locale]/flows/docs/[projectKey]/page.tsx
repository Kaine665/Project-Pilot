...
export default function DocsProjectPage() {
  ...
  // Fetch docs for current project
  const fetchDocs = useCallback(async (key: string) => {
    if (!key) { setDocs([]); return; }
    try {
      const res = await fetch(`/api/docs?project=${encodeURIComponent(key)}`);
      const data = await res.json();
      setDocs(data.docs ?? []);
    } catch {
      setDocs([]);
          ...
          <div className="space-y-2">
            {docs.map(doc => (
              <div
                ...
                onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') handleSelect(doc); }}
                className={`group flex w-full cursor-pointer items-center justify-between rounded-lg border px-5 py-3.5 text-left transition-colors ${
