...
| 2026-04-08 | **Skills 与 AgentSkills/OpenClaw 对齐**：绑定 `type: skill` 时注入完整 `SKILL.md` 正文（非仅摘要）；支持 `disable-model-invocation`；提示词树 Skill 块估算/预览与注入一致 | `skill-loader.ts`、`skill-store.ts`、`routes/prompts.ts`、`types/resource.ts`、`data-storage.md`、`领域与数据.md`、`MEMORY.md`、本文件 |
